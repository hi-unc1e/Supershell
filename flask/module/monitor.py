    ...
    try:
        with rssh_client(key_file, rssh_ip, rssh_port) as rssh_target:
            ...
            if version_result['stat'] == 'success' and who_result['stat'] == 'success':
                ...
                failed_str = 'version: ' + version_result['result'].strip() + '\n' + 'connect number: ' + who_result['result'].strip()
...
def getSupershellVersionInfo(supershell_version_dict):
    '''
        获取Supershell版本信息
    '''
    try:
        version = supershell_version_dict['version']
        description_list = supershell_version_dict['info'][version]['description']
        mtime = supershell_version_dict['info'][version]['mtime']
