...
from const import key_file, rssh_ip, rssh_port, support_os_arch_list, flow_list, supershell_version_dict
...
@compile_view.route('/supershell/compile/select', methods=['POST'])
def get_select():
    '''
        获取客户端支持生成的系统架构数据列表和流量伪装类型接口
    '''
    return jsonify({'support_os_arch_list': support_os_arch_list, 'flow_list': flow_list})
...
def make_client():
    ...
    check_result = check_compile_input(filename, address, port, os_arch, address_proxy, port_proxy, flow, upx, garble)
